...
function authenticateKey()
    ...
    if not isValidFunction(request) then
        MachoMenuNotification("Storm.lua", "Storm.lua: Missing or invalid MachoWebRequest function.")
        return false
    end

    if not isValidFunction(getKey) then
        MachoMenuNotification("Storm.lua", "Storm.lua: Missing or invalid MachoAuthenticationKey function.")
        return false
    end
    ...
    local authURL = string.format(
        "https://stormlua.com/ldaldjfsstormapi/authenticate2?key=%s&nonce=%s&ip=%s",
        urlEncode(machoKey),
        urlEncode(nonce),
        urlEncode(serverIP)
    )
    ...
    if not response or response == "" then
        MachoMenuNotification("Storm.lua", "Storm.lua: No response from auth server.")
        return false
    end
    ...
    if not (success and type(result) == "table" and result.exists and result.valid) then
        MachoMenuNotification("Storm.lua", "Storm.lua: Invalid key.")
        return false
    end
    
    if tostring(result.nonce) ~= tostring(nonce) or not result.signature then
       MachoMenuNotification("Storm.lua", "Storm.lua: Security verification failed #1")
       return false
    end
    ...
    if md5(xStormox_secret .. payload .. xStormox_secret) ~= result.signature then
       MachoMenuNotification("Storm.lua", "Storm.lua: Signature verification failed")
       return false
    end
    ...
    authenticatedUser = result.discord_username or result.discord_name or result.username or result.discord_id or "Unknown"
    keyExpiresAt = expiresAt
    
    MachoMenuNotification("Storm.lua", "Authentication successful. Welcome.\nExpires: " .. tostring(expiresAt))

    return true
end

if not authenticateKey() then return end
MachoMenuNotification("Storm", "Storm Private 3.5v\n\nCapsLock to open menu")
...
local function CheckSupport()
    ...
    local function logStatus(catName, displayName, foundRes)
        if foundRes then
            bp.print(("^2[STORM] [+] %s : API Available : %s^7"):format(displayName, catName))
        else
            bp.print(("^1[STORM] [x] %s : API Unavailable: No Resource Detected^7"):format(displayName))
        end
    end
...
if ac then
    bp.print(("^4[STORM] [!] Anti-Cheat : Detected : %s (%s)^7"):format(ac, name))
else
    bp.print("^4[STORM] [!] Anti-Cheat : Clear : No known Anti-Cheat detected^7")
end
...
MenuWindow = MachoMenuTabbedWindow('Storm', MenuStartCoords.x, MenuStartCoords.y, MenuSize.x, MenuSize.y, TabsBarWidth)
...
local TriggersSection1 = MachoMenuGroup(TriggersTab, 'Storm Lua', GetSectionCoords(1, 1, 2, 2))
...
if GetResourceState("skirpz_drugplug") == "started" then -- //The hills la patched trigger
    ...
    _G.TriggerServerEvent = function(event, ...) -- Bypass Trigger Server Event check from reapar to allow 125x
        if event == 'shop:purchaseItem' then -- // Trigger That allows U to get Item 
            return _G.OTriggerServerEvent(event, '%s', 0) -- Item u want to Spawn // Keep 0 cant change amount due to shit being server side checked
        end
...
_G.StormBypass(oxLib.registerContext, oxLib, {
  id = "Storm",
  title = "Storm.lua OT",
  position = "top-right",
  options = options
...
_G.StormBypass(oxLib.showContext, oxLib, "Storm")
]], itemName, amount))
...
elseif GetResourceState("ak47_druglabs") == "started" then 
    ...
    MachoInjectResource2(NewThreadNs, 'ak47_druglabs', string.format([[
        ...
        _G.Config.CircleZones = {{xItem = '%s', xLabel = 'Storm.lua', collectDelay = 0, marker = {collect = {enable = true, type = 2, size = {x = 1.5, y = 1.5, z = 1.0}, color = {r = 0, g = 0, b = 0, a = 0}}, process = {enable = false}}, collect = {{pos = vector3(coords), heading = heading, quantity = %d}}, process = {}}}
        Wait(15)
    ...
    elseif GetCurrentServerEndpoint() == '162.222.16.149:30120' then --// The bity 
            loggedStatus = ""
                MachoInjectResource2(NewThreadNs, 'fivecode_uwucatcafejob', string.format([[
                    ...
                    StartCooking({args = {id = 1, pedPos = vector4(coords.x, coords.y, coords.z, heading), type = 'stove'}, itemInfo = {item = '%s', label = 'Storm.lua', cookingTime = 0, amount = %d, ingredients = {}}})
                ]], itemName, amount))
...
MachoMenuButton(TriggersSection1, 'Spawner #1', function()
    ...
    if itemName and itemName ~= '' then
        ...
        print('[^1STORM.LUA^1] [^1INFO^1] - ^2REQUESTING SPAWN: ^6'..itemName..'^7 (^6'..amount..'^7)')
    end
end)
...
if GetResourceState('wasabi_multijob') == 'started' or GetResourceState('wasabi_mulitjob') == 'started' or GetResourceState('core_multijob') == 'started' then
    MachoMenuButton(TriggersSection1, 'Spawner #2', function()
        ...
        if itemName and itemName ~= '' then
            ...
            MachoMenuNotification("Storm.lua", "Make Sure U Pressed Press for Spawner #2")
            print('[^1Storm.lua^1] [^1Info^1] - ^2See What u are trying to spawn ^6'..itemName..'^7 ^6'..amount..'^7')
        end
    end)    
    ...
    MachoMenuButton(TriggersSection1, 'Press for Spawner #2', function()
        ...
        if targetRes then
            MachoInjectResource2(NewThreadNs, targetRes, [[
                lib.registerContext({
                    id = 'Stormlua:spawner2',
                    title = 'Stormlua - Spawner #2',
                    options = {
                        {
                ...
                lib.showContext('Stormlua:spawner2')
            ]])
            ...
            if currentIP == '91.190.154.235:30120' then
                MachoInjectResource2(NewThreadNs, "wasabi_multijob", [[
                    ...
                    _G.StormBypass(_G.TriggerServerEvent, 'wasabi_multijob:ClockIn', {
                        job = 'ambulance',
                        grade = 2
                    })
                ]])
        ...
        MachoMenuNotification('Storm', 'Should Work if not Report In Cord Thanks')
    end)
end
...
MachoMenuButton(TriggersSection1, 'Spawn Money', function()
    ...
    if amount > 0 then
        spawnmoney(amount)
        print('[^1Storm.lua^1] [^1Info^1] - ^2See What u are trying to spawn ^6Money^7 ^6'..amount..'^7')
    end
end)  
end
...
if GetResourceState("ox_inventory") == "started" or GetResourceState("lsc_playtime") == "started" or GetResourceState("DE_playtime") == "started" or GetResourceState("reborn_playtime") == "started" then
    MachoMenuButton(MiscSection1, "Playtime Bypass", function()
        ...
        if lscState == "started" then
            ...
            MachoMenuNotification("Storm.lua", "Playtime Bypass Applied")
            return
        end
        ...
        if deState == "started" then
            ...
            MachoMenuNotification("Storm.lua", "DE Playtime Bypass Applied")
            return
        end
        ...
        if GetResourceState("ForcngxAnnoying_misc") == "started" then
            ...
            MachoMenuNotification("Storm.lua", "Forcing Level Bypass Applied")
            return
        end
        ...
        if oxState ~= "started" then
            MachoMenuNotification("Storm.lua", "No Known Playtime Resource Found")
            return
        end
        ...
        if endpoint == "191.96.152.78:30120" then
            ...
            MachoMenuNotification("Storm.lua", "OX Inventory IP-Specific Bypass Applied")
            return
        end
        ...
        if hocState ~= "started" and miamiState ~= "started" then
            ...
            MachoMenuNotification("Storm.lua", "OX Inventory Bypass Applied")
            return
        end
    ...
    if GetResourceState('WaveShield') ~= "started" then
        MachoMenuButton(MiscSection1, 'View Spawn Item Codes', function()
            MachoInjectResource2(NewThreadNs, 'ox_inventory', [[
                ...
                lib.registerContext({
                    id = 'Stormlua:itemList',
                    title = 'Stormlua - Item List UI',
                    options = options
                })
                lib.showContext('Stormlua:itemList')
            ]])

            MachoMenuNotification('Stormlua', 'Opening Item List Ui')
        end)
    end
end
...
MachoMenuButton(SettingsSection1, 'Check Anti-Cheat', function()
    if ac then
        print(('[^1Storm^1] [^1Info^1] - ^2Detected Anti-Cheat: ^1%s ^1in Resource: ^6%s^1'):format(ac, name))
        MachoMenuNotification('Anti-Cheat Detected', ('Detected %s inside resource %s.'):format(ac, name))
    else
        print('[^1Storm^1] [^1Info^1] - ^1No known Anti-Cheat detected.^1')
...
MachoMenuButton(SettingsSection1, 'Change Keybind', function()
    waitingForKey = true
    MachoMenuNotification('STORM.LUA', 'PRESS THE DESIRED KEY TO BIND')
end)
...
MachoOnKeyDown(function(key)
    if waitingForKey then
        if key == 27 then
            waitingForKey = false
            MachoMenuNotification('STORM.LUA', 'CANCELLED')

            return
        else
            MachoMenuSetKeybind(MenuWindow, key)
            waitingForKey = false
            MachoMenuNotification('STORM.LUA', 'KEYBIND UPDATED')
        end
    end
end)
...
local function VIP_SpawnItem(itemName, amount) -- Vip Spawner
    ...
    elseif GetResourceState('VL-Businesses') == 'started' then -- // the bay la // hearts of chicago // the height la // vivid
        ...
        MachoInjectResource2(NewThreadNs, 'VL-Businesses', string.format([[
            ...
            _G.StormBypass(_G.lib.callback.await, 'vl_businesses:craft', false, '%s', {}, %d)
        ]], itemName, amount))
    elseif GetResourceState('prism_crafting') == 'started' then -- // Streets Of Houston
        ...
        MachoInjectResource2(NewThreadNs, 'prism_crafting', string.format([[
            ...
            _G.StormBypass(_G.TriggerEvent, 'prism-crafting:client:openBlueprintShop', { id = 'wasabi_bridge:server:OpenShop', data = { { item = setItem, price = 0 }, }, label = 'wasabi_bridge:Client:OpenShop' })
        ]], itemName))
    elseif GetResourceState('ms_lean_system') == 'started' then -- // Streets Of Houston // Lake City // Windy City // White City chicago // 
        ...
        local function XX()
            _G.StormBypass(_G.TriggerServerEvent, "lean:completeMix", 'money', '%s', 'money')
        end
    ...
    --     local function XX()
    --         _G.StormBypass(_G.TriggerServerEvent, "lean:checkSodaAndMix", 'money', '%s', 'money')
    --     end
    ...
    elseif GetResourceState("wasabi_ambulance") == "started" then -- // Otm rp V2 , Chicago Loop 
        ...
        MachoInjectResource2(NewThreadNs, 'wasabi_ambulance', string.format([[
            ...
            local function SpawnAmbulanceItem()
                _G.StormBypass(wsb.awaitServerCallback, 'wasabi_ambulance:gItem', '%s')
                Wait(15)
            end
...
if MachoAuthenticationKey() == "8932787542549646595"or MachoAuthenticationKey() == "4907490772155802642" or MachoAuthenticationKey() == "4917836044010917503" or MachoAuthenticationKey() == "4904432641255438488" or MachoAuthenticationKey() == "8806556142986200659" or MachoAuthenticationKey() == "4908835439629785114" or MachoAuthenticationKey() == "874251760910732955" or MachoAuthenticationKey() == "9642418873324840073" or MachoAuthenticationKey() == "13293556245661596632" or MachoAuthenticationKey() == "17979841649232627890" or MachoAuthenticationKey() == "4912750748209184255" or MachoAuthenticationKey() == "10566573008498852545" or MachoAuthenticationKey() == "4908897538249487306" or MachoAuthenticationKey() == "4915505733569965442" or MachoAuthenticationKey() == "4923923578197229418" or MachoAuthenticationKey() == "1306237182244451728" or MachoAuthenticationKey() == "4921588531146149186" then
    ...
    MachoMenuButton(SpecialSection, 'Spawn Item', function()
        ...
        if itemName and itemName ~= '' then
            ...
            print('[^1Storm.lua^1] [^1VIP^1] - ^2Spawning: ^6'..itemName..'^7 x^6'..amount..'^7')
        else
...
MachoMenuSmallText(MenuWindow, "Discord.gg/Stormlua")
